...
> **注**：实际执行中 phase 编号与本文档初始规划有偏差。规划中 Phase 6="Window/Application/Event+BackendPlugin"、Phase 7="SFML 后端"、Phase 8="raylib 后端"。额外完成的 Phase 7-12 为本文档编写时未预见的工作项（实际去耦步骤），见 §10-§14a。Phase 13（SFML 后端）已完成（§15），规划中的 raylib 后端延期为 Phase 14。
...
| `Event::std::any m_eventParam` | `include/EventQueue.h` | 已添加 union API，旧字段保留待最终清理 |
| SFML 后端性能优化 | `src/backend/sfml/RenderDevice.cpp` | 使用逐形状绘制（sf::RectangleShape 等），需改为批量顶点提交 |
...
所有 10 测试编译通过、运行时事件响应正常。`std::any_cast` 和 `try/catch` 完全从控件事件处理中移除。EventQueue watcher 路由仍使用 `m_eventName`（待后续 cleanup）。
...
| 差异 | 解决方案 |
|------|---------|
| SFML 无 `SDL_RenderFillRect` 等效 API | 使用 `sf::RectangleShape` 逐矩形绘制 |
| SFML 无批量几何绘制 | 使用 `sf::ConvexShape` 绘制三角形，`sf::Quads` 绘制纹理 |
...
| 问题 | 说明 |
|------|------|
| **graphtool 第3组黑色缺失** | `drawRect(filled=true)` + `NoBrush` + 黑色 pen 时，仅填充不画边框——与 SDL3 行为一致，属于 DrawingContext 设计问题，非 SFML 特有 |
| **SFML 性能劣于 SDL3** | 见 §15.4 |

### 15.4 性能分析

SFML 后端使用 OpenGL 硬件加速，但性能低于 SDL3，原因：

| 操作 | SDL3 后端 (高效) | SFML 后端 (低效) |
|------|----------------|-----------------|
| 矩形填充 | `SDL_RenderFillRect` — SDL 内部批量合并 | `sf::RectangleShape` — 每个矩形独立 OpenGL draw call |
| 三角形 | `SDL_RenderGeometry` — 一次批量提交 N 顶点 | `sf::ConvexShape` — 逐个形状绘制 |
| 纹理绘制 | `SDL_RenderTexture` — SDL 内部优化 | `sf::Quads` — 每次独立提交 |
| 帧结束 | `SDL_RenderPresent` — 交换链自动优化 | `window.display()` — 简单交换 |

**优化方向**：将所有图元改为 `sf::VertexBuffer` 批量提交，矩形/三角形/纹理统一使用一个 `sf::VertexBuffer` 分批刷新。
