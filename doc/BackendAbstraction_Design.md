...
> **注**：实际执行中 phase 编号与本文档初始规划有偏差。规划中 Phase 6="Window/Application/Event+BackendPlugin"（BackendPlugin 已完成，其余未开始）、Phase 7="SFML 后端"、Phase 8="raylib 后端"。额外完成的 Phase 7-11 为本文档编写时未预见的工作项，见 §10-§14。
...
| 6 — Window / Application | ⏳ **待开始** | BackendPlugin 系统已单独完成（C ABI 工厂 + CMake BACKEND_PLUGIN 选项）；Window/InputBackend/Event 迁移未开始 |
...
| `ResourceProvider` | `include/ResourceProvider.h` | `src/ResourceProvider.cpp` | ✅ 同上 |
| `Cursor` | `include/Cursor.h` | `src/Cursor.cpp` | ✅ 同上 |
...
| 6 | Window / App / Event 重构 | ~15 | ~600 | 3-4 | ⏳ 待开始 | ★★ |
...
| 12 | SFML 后端 | ~5 | ~1500 | 5-7 | ⏳ 待开始 | ★★★★ |
| 13 | raylib 后端 | ~5 | ~1150 | 5-7 | ⏳ 待开始 | ★★★★ |
| **合计** | | **~120** | **~4828** | **31-43** | | |
...
Phase 5  (Font/TextRenderer)  ← ✅ 已完成 — Label/EditBox/TextArea 全部迁移
    ↓
Phase 6  (Window/App/Event)   ← ⏳ 待开始 — BackendPlugin 已提前完成（C ABI 工厂 + 插件加载）
    ↓
...
Phase 11 (移除 SDL_Renderer)  ← ✅ 已完成 — ControlBase.h 零 SDL 依赖
    ↓
Phase 12 (SFML backend)       ← ⏳ 待开始
    ↓
Phase 13 (raylib backend)     ← ⏳ 待开始
```

> **注**：Phase 4（SDL 头文件解耦）在 Phase 3 后自然衍生——Texture/Surface 抽象完成后，大量头文件不再需要 SDL 类型，可直接移除或替换为具体子头文件。Phases 7-11 为执行过程中识别出的额外去耦机会，均在 Phase 5 后顺次完成。
